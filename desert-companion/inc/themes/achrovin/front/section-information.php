...
?>
<section id="dt_information" class="dt_service dt_service--fourteen dt-py-default front-info">
		...
		<div class="dt-row dt-g-4">
			<?php
				...
				foreach ( $corpiva_information_option as $i=>$item ) {
					...
					$button = ! empty( $item->text2 ) ? apply_filters( 'corpiva_translate_single_string', $item->text2, 'Information section' ) : '';
					...
					<div class="item-inner">
						<div class="item-content">
							<div class="item-meta">
								...
								<h2 class="count"><?php echo esc_html(str_pad($i + 1, 2, '0', STR_PAD_LEFT)); ?></h2>
							</div>
							<?php if ( ! empty( $title ) ) : ?>
							<h5 class="item-title dt-mb-2"><?php echo wp_kses( html_entity_decode( $title ), $allowed_html ); ?></h5>
							<?php endif; ?>								
							<?php if ( ! empty( $text ) ) : ?>
						...
						<?php if ( ! empty( $button ) ) : ?>
						<a class="dt-more" href="<?php echo esc_url($link); ?>"><?php echo wp_kses( html_entity_decode( $button ), $allowed_html ); ?> <i class="fal fa-arrow-right" aria-hidden="true"></i></a>
						<?php endif; ?>
					</div>
				</div>
			<?php } } ?>
